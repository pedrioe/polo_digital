 ...
 */http://local:8000/carrusel?total=3
...
app.post("/registro", function(request, response) {
    ...
    connection.query(`insert into empleados_clientes ( nombre, apellidos, usuarioId) values ("${nombre}", "${apellidos}", "${usuarioId}")`, function(error, result, fields){
        handleSQLError(response, error, result, function(result) { 
            console.log(result);
...
app.post("/clientes/:id" , function(resquest, response){
    let nombre = request.body.nombre;
    ...
    const idclientes= request.params.idEvento;
    
    console.log(request.body);
    connection.query(`update clientes set nombre = "${nombre}", razonSocial = "${razonSocial}", cif = "${cif}", telefono = "${telefono}", sector = "${numeroEmpleados}" where id = 1`)
